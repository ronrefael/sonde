...
fn windsurf_data_dir(cfg: &SondeConfig) -> PathBuf {
    if let Some(wcfg) = cfg.windsurf.as_ref() {
        if let Some(dir) = wcfg.sessions_dir.as_deref() {
            return config::expand_tilde(dir);
        }
    }

    // Check common Windsurf/Codeium data locations
    if let Some(home) = dirs::home_dir() {
        let codeium = home.join(".codeium");
        if codeium.exists() {
            return codeium;
        }
        #[cfg(target_os = "macos")]
...
fn cost_from_usage_json(dir: &Path) -> Option<f64> {
    ...
    if let Ok(entries) = serde_json::from_str::<Vec<WindsurfUsage>>(&content) {
    ...
    if let Ok(entry) = serde_json::from_str::<WindsurfUsage>(&content) {
        let model = entry.model.as_deref().unwrap_or("claude-3.5-sonnet");
        let input = entry.input_tokens.unwrap_or(0);
        let output = entry.output_tokens.unwrap_or(0);
...
fn calculate_session_cost(path: &Path) -> Option<f64> {
    ...
    for line in content.lines() {
        ...
        let event: WindsurfEvent = match serde_json::from_str(line) {
            Ok(e) => e,
            Err(_) => continue,
        };

        match event {
            WindsurfEvent::ModelUsage {
                model,
                input_tokens,
                output_tokens,
                ..
...
pub fn render(_ctx: &Context, cfg: &SondeConfig) -> Option<String> {
    ...
    let dir = windsurf_data_dir(cfg);
    if !dir.exists() {
        tracing::debug!("windsurf: data dir does not exist: {}", dir.display());
        return None;
    }

    if let Some(cost) = cost_from_usage_json(&dir) {
        let text = format!("Windsurf: ${cost:.2}");
    ...
    let session = match latest_session(&dir) {
        Some(s) => s,
        None => {
            tracing::debug!("windsurf: no session files found");
            return None;
        }
    };
...
mod tests {
    ...
    #[test]
    fn parse_windsurf_usage_fixture() {
        let json = r#"[{"model":"claude-3.5-sonnet","input_tokens":10000,"output_tokens":5000}]"#;
        let entries: Vec<WindsurfUsage> = serde_json::from_str(json).unwrap();
        assert_eq!(entries.len(), 1);
        let cost = token_cost(
            entries[0].input_tokens.unwrap(),
            entries[0].output_tokens.unwrap(),
            entries[0].model.as_deref().unwrap(),
        );
        assert!(cost > 0.0);
    }
    ...
    fn returns_none_when_disabled() {
        ...
        cfg.windsurf = Some(crate::config::WindsurfConfig {
            enabled: Some(false),
            sessions_dir: None,
        });
    ...
    fn returns_none_when_dir_missing() {
        ...
        cfg.windsurf = Some(crate::config::WindsurfConfig {
            enabled: Some(true),
            sessions_dir: Some("/nonexistent/windsurf/path".to_string()),
        });
